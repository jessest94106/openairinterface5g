...
void threadCreate(pthread_t* t, void * (*func)(void*), void * param, char* name, int affinity, int priority)
{
  ...
  // Naming a thread is purely cosmetic (for debugging). In environments where
  // RLIMIT_RTPRIO=0, SCHED_FIFO threads can race such that the TID is briefly
  // unresolvable (ESRCH) at setname time. Do not crash the whole modem over a
  // failed thread name — just warn.
  if (ret != 0)
